...
int lab2_close(int fd) {
    if (fd < 0 || fd >= 256 || openFiles[fd] == NULL) return -1;
...
SSIZE_T lab2_read(int fd, void *buf, size_t count) {
    ...
    if (file->cacheTail) file->cacheTail->next = newEntry;
    file->cacheTail = newEntry;
    if (!file->cacheHead) file->cacheHead = newEntry;
    file->cacheCount++;
...
long lab2_lseek(int fd, long offset, int whence) {
    if (fd < 0 || fd >= 256 || openFiles[fd] == NULL) return -1;
    LARGE_INTEGER li;
    li.QuadPart = offset;
